...
public class UsersService implements RegisterUserUseCase {
      ...
      public Mono<User> registerUser(User user) {
            ...
            return this.userRepository.existsByDni(user.getDni()).filter(exists -> !exists).switchIfEmpty(Mono.error(new UserAlreadyExistsException("Ya existe un usuario con ese dni registrado"))).then(registerAccessUseCase.create(user)).map(acc -> {
                  user.setAccess(acc);
                  return user;
            }).flatMap(user1 -> {
      ...
      public Mono<User> registerTeacher(User user) {
            ...
            return this.userRepository.existsByDni(user.getDni()).filter(exists -> !exists).switchIfEmpty(Mono.error(new UserAlreadyExistsException("Ya existe un usuario con ese DNI registrado"))).then(registerAccessUseCase.create(user))
